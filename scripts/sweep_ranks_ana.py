...
frames = []
for frame in sorted(os.listdir(osp.join(args.data_dir, args.scene, 'meshes'))):
...
for min_tsdf, max_tsdf in args.trunc_values:
    ...
    for frame in tqdm.tqdm(frames):
        sdf = torch.load(osp.join(args.data_dir, args.scene, 'meshes', frame))['sdf']
        ...
        for tt_rank in reversed(sorted(args.tt_ranks)):
            ...
            result[(min_tsdf, max_tsdf)][rank][tt_rank] = {
                ...
                'metrics': compute_metrics(
                    [(osp.join(args.data_dir, args.scene, 'meshes', frame),
                      osp.join(args.data_dir, args.scene, 'meshes', frame[4:-2] + 'obj'))
                     for frame in frames],
                    local_res_tt,
                    preprocessing_fn,
                    min_tsdf, max_tsdf,
